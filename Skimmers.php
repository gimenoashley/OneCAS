<?php
	$servername = "localhost";
  	$username = "root";
  	$password = "";
  	$dbname = "acadorgdatabase";

	// Create connection
  	$conn = new mysqli($servername, $username, $password, $dbname);

	$sql2 = "SELECT * FROM acadorganizations WHERE id = 5";
  	$results = $conn->query($sql2);
  	$sql = "SELECT * FROM acadofficers WHERE id = 5";
  		...
  		while ($row = $results->fetch_assoc()) {
  			echo "<div class='about'>".
				...
				"<h2>VISION</h2>"."<p>".$row["vision"]."</p>".
  				"<h2>CONTACT</h2>"."<td>".$row["email"]."<br>".
  				$row["facebook"]."<br>".$row["twitter"]."</td>"
  				."</div>";
